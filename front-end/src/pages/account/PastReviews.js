      ...
      <Comment.Group>
        {this.props.reviews.map((review) =>
          <Comment key={review.id}>
            <Comment.Avatar style={{cursor: 'pointer'}} onClick={() => {
              if (review.reviewer.id) this.props.history.push('/profile/' + review.reviewer.id)
            }} src={'/images/avatar/' + review.reviewer.avatar + '.jpg'}/>
            <Comment.Content>
              <Comment.Author style={{cursor: 'pointer'}} onClick={() => {
                if (review.reviewer.id) this.props.history.push('/profile/' + review.reviewer.id)
              }}> {review.reviewer.name} </Comment.Author>
