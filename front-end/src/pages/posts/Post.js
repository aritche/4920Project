...
import {Grid, Item, Label, Rating} from 'semantic-ui-react';
      ...
      <Item key={post.id} as={Link} to={"/posts/" + post.id}>
        ...
        <Item.Content>
          <Item.Header>{post.title}
            <Label color={post.status === 'ACCEPTED' ? 'green' : post.status === 'CLOSED' ? 'red' : 'blue'}
          ...
          <Item.Meta>
            ...
            <Rating size={'medium'} icon='star' style={{marginLeft: '10px'}} defaultRating={post.movee.rating_overall}
          ...
          <Item.Description>
            <Grid>
              ...
              <Grid.Column width={7}>
                <div style={{
                  maxHeight: '90px', overflow: 'hidden', textOverflow: 'ellipsis',
                  whiteSpace: 'nowrap'
                }}>
